...
function scoreRecommendation(
  ...
  return (
    (item.featured ? 5 : 0) +
    (item.editorPick ? 4 : 0) +
    Math.min(item.views || 0, 1000) / 100
  );
}
...
function scoreRelatedContent(
  ...
  if (matches(item.genre, query.genre)) {
    score += 4;
  }

  if (matches(item.type, query.type)) {
    score += 2;
  }

  if (
    matches(item.creatorName, query.creatorName)
  ) {
    score += 3;
  }
...
export function selectPublicContent(
  ...
  const cleanContent = content
    .filter((item) => item.id !== query.excludeId)
  ...
  if (query.mode === "editor_picks") {
    result = cleanContent.filter(
      (item) => item.editorPick
    );
  }

  if (query.mode === "genre" && query.genre) {
    result = cleanContent.filter((item) =>
      matches(item.genre, query.genre)
    );
  }

  if (query.mode === "type" && query.type) {
    result = cleanContent.filter((item) =>
      matches(item.type, query.type)
    );
  }
  ...
  if (query.mode === "because_you_watched") {
    result = cleanContent
      .filter(
        (item) =>
          matches(item.type, query.type) ||
      ...
      .sort(
        (a, b) =>
          scoreRelatedContent(b, query) -
          scoreRelatedContent(a, query)
      );
  }

  if (query.mode === "hidden_gems") {
    result = cleanContent
  ...
  if (query.mode === "recommended") {
    result = shuffle(cleanContent).sort(
      (a, b) =>
        scoreRecommendation(b) -
        scoreRecommendation(a)
    );
  }
